...
const ListKota = () => {
  ...
  const getKota = async () => {
    const response = await axiosJWT.get("http://localhost:8000/kota", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  const deleteKota = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/kota/${id}`);
    } catch (error) {
      console.log(error);
    }
    Swal.fire({
          ...
          <table className="table is-striped is-fullwidth">
            <thead>
              <tr>
                ...
                {/* <th>Gambar</th> */}
                <th>
                  <button className="add-btn" onClick={() => goToAddKota()}>
              ...
              {kota.map((kota, index) => (
                <tr key={kota.id}>
                  ...
                  {/* <td className="parent-img">
                    <img
                      className="img-class"
                      src={"http://localhost:8000/images/" + kota.image}
                    />
                  </td> */}
                  <td>
                    <button
                      className="delete-btn"
                      onClick={() => deleteKota(kota.id)}
                    >
                      <FontAwesomeIcon icon={faTrash} />
                    </button>
                    <button className="edit-btn">
                      <Link to={`/edit-kota/${kota.id}`}>
